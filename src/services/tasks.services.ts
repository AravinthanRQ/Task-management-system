...
export const taskService = {
    async createTask(projectId: number, userId: number, taskDto: CreateTaskDto): Promise<Task> {
    ...
    async getTasksByProject(projectId: number, role: userRole, userId: number): Promise<Task[]> {
        ...
        const tasks = role === userRole.ADMIN
    ...
    async updateTaskStatus(taskId: number, userId: number, role: userRole, taskDto: UpdateTaskDto): Promise<Task> {
        ...
        if (role === userRole.USER && task.createdBy.id !== userId) {
            throw new HttpError("Unauthorized to update this task", HttpStatusCode.FORBIDDEN);
        }
